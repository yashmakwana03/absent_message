...
class _DepartmentSetupScreenState extends State<DepartmentSetupScreen> {
  ...
  Future<void> _loadExistingDepartments() async {
    ...
    if (existingDepts.isNotEmpty) {
      setState(() {
        _deptFields = existingDepts.map((dept) => {
          'id': dept.id,
    ...
    if (deptId != null) {
      final confirm = await showDialog<bool>(
        ...
        builder: (ctx) => AlertDialog(
          ...
          content: const Text("This will remove the department from the database immediately. Students in this department might be affected."),
          actions: [
  ...
  Future<void> _saveDepartments() async {
    ...
    for (var field in _deptFields) {
      ...
      if (id != null) {
        // Update Existing
        await DatabaseHelper.instance.updateDepartment(Department(id: id, name: name));
      } else {
        // Create New
